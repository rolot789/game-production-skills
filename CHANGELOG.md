...
- **`representation_strategies` in `toolkit-contract.yaml`.** The AssetSpec enum offers eight strategies; four of them (`generated_vector`, `generated_3d`, `procedural`, `shader_or_particle`) had no tooling behind them, so selecting one was a valid plan that dead-ended silently at normalization. They are now classified as `planning_only`: choosing one stays legitimate, advancing an asset on one is rejected, and the contract records what each would actually need. `validate_contracts.py` fails if the enum and the classification ever disagree.
- **`contracts/providers.yaml`** — the generation provider capability contract, and `check_provider.py` to enforce it before any image budget is spent. Capability is not a plumbing detail here: `derivation_mode: parent_derived` means "make this, posed differently, preserving identity", which is an image-conditioned edit. A text-to-image-only provider cannot do it and produces a new drawing per family member — exactly the `IDENTITY_DRIFT` the anti-drift machinery exists to prevent. `generation_budget.stop_conditions` already named "provider/topology inadequacy" and had nowhere to record it. The file ships capability *profiles* rather than named vendors, because a vendor list goes stale silently and this repository would then be asserting capabilities it never measured; a project declares its own `generation.capabilities` for anything else. No credentials live there. A shortfall is a `BLOCKER` and explicitly not a generator defect — the contract compiled correctly and the provider cannot execute it.
- **`run_model_evals.py`** — the harness the `triggering` and `gates` suites needed. They were specified and left unrun, which made "the trigger-shaped descriptions improve skill selection" a hypothesis rather than a result. Backends are pluggable (`anthropic`, any CLI via `command`, `stub` for self-testing), so the repository is not pinned to one provider. Scoring is deterministic: every case asks for a small JSON object and compares fields, because a harness that greps prose for encouraging words reports a score the skills did not earn. Without a backend it exits `2` and says `SKIPPED`, never `PASS`. Triggering cases see only skill frontmatter, since that is what a real runtime selects on — feeding the whole body would measure a different mechanism and score better than the product does.
- **The model evals have a real score.** One run, `claude-sonnet-5` via `claude -p` with tool use disabled (the model sees only prompt text, no filesystem access): `triggering` 12/12, `gates` 9/10 against the default `game-art-production-orchestrator` body. The one gates failure needed a G-level rule that lives in `game-asset-generator/SKILL.md`; it passes when re-run with `--gate-skill game-asset-generator`, confirming the gap is which skill body the harness quoted, not a missing rule. Running it for real also found a genuine bug it was built to catch: `GAT-006` expected the literal substring `coherent lineage`, and the orchestrator's actual wording is `coherent current lineage` — the exact-match scorer working as designed against a case typed slightly wrong. Fixed to `cites: coherent`. See `ARCHITECTURE.md` §10.
- **A scorer self-test in CI.** CI cannot run a model, so `run_evals.py` now runs the harness against two fixtures: one matching every expectation, one with three deliberately wrong answers. A scorer that would accept the wrong run fails the build. Verified by sabotaging the scorer and watching the build go red.
- **Animation clips.** `animation` appeared zero times in the contract and zero times in the AssetSpec schema, while five skills already named animation as a requirement in prose. A clip is now modelled as what it is — an ordered family — so nothing about family coherence, canonical-parent derivation, or shared scale had to be reinvented: each frame is its own asset sharing a `family_id`, and an `animation` block carries the ordering (`clip_id`, `frame_index`, `frame_count`, `fps`, `loop`, `canonical_frame`).
  - **`check_frames.py`** verifies a clip as a sequence: pivot drift (the bob), canvas mismatch (the jump), content sliding inside a fixed canvas, duplicate frames (the stutter), missing indices, and loop closure. Every one of these is invisible frame by frame and obvious the moment the clip plays.
  - **`pack_atlas.py` emits an ordered frame map** — frames with rects, `fps`, `loop`, `hold_frames`, `duration_ms` — which is the difference between an atlas and a sprite sheet an engine can play. It refuses to pack a partial clip rather than emit a frame map indexing frames the sheet does not contain.
  - **The worked example ships a real 4-frame opening clip**, packed as a 260×520 sheet, with the continuity report and every hash produced by the actual tools.
  - **`poses_share_a_subject` is measured and deliberately not decided.** The obvious check — "a frame too far from the canonical pose was redrawn rather than posed" — does not work: on this example a legitimate end-of-travel frame sits 0.52 from the canonical pose while a frame replaced with an entirely different shape sits 0.47. The redraw is *closer*, and core-coverage and monotonicity variants invert the same way. Silhouette distance cannot separate the two when the moving part is a large share of the shape, so the script reports `delta_from_canonical` and leaves the verdict to whoever can compare against the family's `must_preserve` list. A threshold tuned to pass here would have misled on every clip shaped differently.
...
- **Schemas are mirrored per skill instead of wholesale.** Every skill carried all 11 schemas — 42 KB, roughly 10,000 tokens — while naming exactly one, so an agent that globbed `references/` paid for ten it had no use for. A skill now gets the schemas for what it writes, plus the handoff envelope when it routes, plus `_defs.json`. Mirrored schema files drop from 77 to 25 and `skills/` from 476 KB to 156 KB of schemas; `sync_contracts.py` deletes orphaned copies, and `validate_contracts.py` fails the build when a skill names a schema the manifest does not mirror into it.
- **`contracts/routing-index.yaml`** — the compact lookup, generated from `routing.yaml` so it cannot disagree with the table it indexes. Seven skills were each told to read 279 lines to resolve one row on every rework cycle; the index is 140 lines and 57% fewer tokens. The full table stays authoritative and is what you open for a row's description or note, for scope escalation, `multi_owner`, or `qc_escape`.
